...
if ($stmt = $conn->prepare('SELECT id, pass FROM users WHERE username = ?')) {
    ...
    if ($stmt = $conn->prepare('INSERT INTO users (username, pass, name, vorname, role ) VALUES (?, ?, ?, ?, ?)')) {
        // We do not want to expose passwords in our database, so hash the password and use password_verify when a user logs in.
        $password = password_hash($_POST['pass'], PASSWORD_DEFAULT);
        $stmt->bind_param('sssss', $_POST['username'], $password, $_POST['name'], $_POST['vorname'], $_POST['role']);
        $stmt->execute();
